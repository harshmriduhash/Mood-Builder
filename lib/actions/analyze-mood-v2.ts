...
export async function analyzeMoodWithSolarPro(
  content: string
...
`;
    ...
    console.log(
      "Solar Pro API response received:",
    ...
    try {
      ...
      console.error(
        "Failed to parse JSON directly, trying to extract JSON from text:",
        e
      );
    ...
    return {
      ...
      summary:
        "The mood appears neutral. Unable to perform detailed analysis due to technical issues.",
    };
  }
}

export async function saveAnalyzedJournalEntry(
  content: string,
  ...
  try {
    ...
    const formattedAnalysis = {
      ...
      emotions: Array.isArray(cleanAnalysis.emotions)
        ? cleanAnalysis.emotions
        : [],
    ...
    // Insert the journal entry with the full analysis data into the new v2 table
      ...
      for (const emotionName of formattedAnalysis.emotions) {
        try {
          ...
          if (existingEmotion) {
            ...
            console.log(
              `Found existing emotion: ${emotionName} with ID: ${emotionId}`
            );
          } else {
            // Create new emotion
            ...
            if (createError) {
              console.error(
                `Error creating emotion ${emotionName}:`,
                createError
              );
              continue;
            }
            ...
            console.log(
              `Created new emotion: ${emotionName} with ID: ${emotionId}`
            );
          }

          // Link emotion to entry using the new v2 junction table
          console.log(`Linking emotion ${emotionName} to entry ${data.id}`);
          const { error: linkError } = await supabase
            .from("entry_emotions_v2")
            .insert({
              entry_id: data.id,
          ...
          if (linkError) {
            console.error(
              `Error linking emotion ${emotionName} to entry:`,
              linkError
            );
          }
        } catch (err) {
      ...
      for (const themeName of formattedAnalysis.themes) {
        try {
          ...
          if (existingTheme) {
            ...
            console.log(
              `Found existing theme: ${themeName} with ID: ${themeId}`
            );
          } else {
            // Create new theme
          ...
          // Link theme to entry using the new v2 junction table
          console.log(`Linking theme ${themeName} to entry ${data.id}`);
          const { error: linkError } = await supabase
            .from("entry_themes_v2")
            .insert({
              entry_id: data.id,
              theme_id: themeId,
            });
          ...
          if (linkError) {
            console.error(
              `Error linking theme ${themeName} to entry:`,
              linkError
            );
          }
        } catch (err) {
